...
function atmFunction()
{
    ...
    let currentUser = accountData.filter((data) => accountNumber == data.accountNumber);
   ...
   else
   {
    ...
    if(cardNumber == currentUser[0]["cardNumber"])
    {
        ...
        {
           let choice = window.prompt("Enter 1 to withdraw and 2 to deposit the money");
           if(choice == 1)
           {
                withDraw();
           }
   ...
   function withDraw()
   {
        ...
        {
            currentUser[0]["accountBalance"] = currentUser[0]["accountBalance"] - requestedAmount;
   ...
   function deposit()
   {
    ...
    {
        currentUser[0]["accountBalance"] = Number(currentUser[0]["accountBalance"]) + Number(depositAmount);
